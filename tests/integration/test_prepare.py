...
def test_prepare_file_creates_train_and_valid(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["prepare", "file", "--input", str(input_path), "--output", str(output_dir)])
    ...
    valid_rows = [json.loads(line) for line in (output_dir / "valid.jsonl").read_text().splitlines() if line.strip()]
...
def test_prepare_file_respects_max_samples(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["prepare", "file", "--input", str(input_path), "--output", str(output_dir), "--max-samples", "10"],
    )
    assert result.exit_code == 0
...
def test_prepare_file_respects_seed(tmp_path: Path) -> None:
    ...
    out_a = tmp_path / "a"
    out_b = tmp_path / "b"
    runner.invoke(app, ["prepare", "file", "--input", str(input_path), "--output", str(out_a), "--seed", "42"])
    runner.invoke(app, ["prepare", "file", "--input", str(input_path), "--output", str(out_b), "--seed", "42"])
...
def test_prepare_file_deterministic_with_same_seed(tmp_path: Path) -> None:
    ...
    out_a = tmp_path / "a"
    out_b = tmp_path / "b"
    runner.invoke(app, ["prepare", "file", "--input", str(input_path), "--output", str(out_a), "--seed", "1"])
    runner.invoke(app, ["prepare", "file", "--input", str(input_path), "--output", str(out_b), "--seed", "2"])
    assert (out_a / "train.jsonl").read_text() != (out_b / "train.jsonl").read_text()
...
def test_prepare_swe_refuses_test_split() -> None:
    ...
    assert result.exit_code != 0
    assert "evaluation patches would leak" in result.output or "evaluation patches would leak" in (result.stderr or "")
...
def test_prepare_swe_filters_by_max_chars(tmp_path: Path) -> None:
    ...
    assert result.exit_code != 0
    assert "not enough valid SWE examples" in result.output or "not enough valid SWE examples" in (result.stderr or "")
...
def test_prepare_code_filters_by_language(tmp_path: Path) -> None:
    ...
    rows = [
        {"language": "PYTHON", "description": "x" * 200, "solution": "def f(): pass" + "  y" * 30},
        {"language": "CPP", "description": "x" * 200, "solution": "int main() {}" + "  y" * 30},
        {"language": "PYTHON", "description": "x" * 200, "solution": "def g(): pass" + "  y" * 30},
    ]
    fake_dataset = MagicMock()
...
def test_prepare_all_writes_valid_messages(tmp_path: Path) -> None:
    ...
    with patch("datasets.load_dataset", return_value=fake_dataset):
        ...
        assert result.exit_code == 0
        written = [json.loads(line) for line in (tmp_path / "out.jsonl").read_text().splitlines() if line.strip()]
        assert written == rows
...
def test_prepare_evaluate_writes_lcb_prompts(tmp_path: Path) -> None:
    ...
    fake_split = [
        {"question_id": "q1", "question_content": "solve x", "contest_date": "2024", "difficulty": "easy"},
        {"question_id": "q2", "question_content": "solve y", "contest_date": "2024", "difficulty": "hard"},
    ]
    ...
    with patch("datasets.load_dataset", return_value=fake_dataset):
        ...
        assert result.exit_code == 0
        written = [json.loads(line) for line in (tmp_path / "lcb.jsonl").read_text().splitlines() if line.strip()]
