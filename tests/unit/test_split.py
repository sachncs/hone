...
def jsonl(path: Path, count: int) -> None:
...
def test_splitter_same_seed_produces_same_split() -> None:
    samples = examples(20)
    train_a, valid_a = Splitter(0.25, seed=42).split(samples)
    train_b, valid_b = Splitter(0.25, seed=42).split(samples)
...
def test_splitter_different_seeds_produce_different_splits() -> None:
    samples = examples(50)
    train_a, _ = Splitter(0.25, seed=42).split(samples)
    train_b, _ = Splitter(0.25, seed=43).split(samples)
...
def test_splitter_validation_has_at_least_min_valid() -> None:
    samples = examples(20)
    _, valid = Splitter(0.05, seed=42).split(samples)
...
def test_splitter_preserves_all_elements() -> None:
    ...
    assert sorted(id(e) for e in recovered) == sorted(id(e) for e in samples)


def test_partition_returns_counts(tmp_path: Path) -> None:
...
def test_partition_preserves_all_lines_byte_identical(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 40)
    partition(source, tmp_path / "train.jsonl", tmp_path / "valid.jsonl", 0.1, seed=42)
    recovered = lines(tmp_path / "train.jsonl") + lines(tmp_path / "valid.jsonl")
    assert sorted(recovered) == sorted(lines(source))


def test_partition_partitions_disjoint(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 40)
    partition(source, tmp_path / "train.jsonl", tmp_path / "valid.jsonl", 0.1, seed=42)
    train = set(lines(tmp_path / "train.jsonl"))
    valid = set(lines(tmp_path / "valid.jsonl"))
    assert train.isdisjoint(valid)


def test_partition_same_seed_produces_same_split(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 40)
    partition(
    ...
    assert lines(tmp_path / "a" / "train.jsonl") == lines(
        tmp_path / "b" / "train.jsonl"
    )
    assert lines(tmp_path / "a" / "valid.jsonl") == lines(
        tmp_path / "b" / "valid.jsonl"
    )


def test_partition_different_seeds_produce_different_splits(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 50)
    partition(
        source, tmp_path / "a" / "train.jsonl", tmp_path / "a" / "valid.jsonl", 0.1, 42
...
def test_partition_valid_has_at_least_min_valid(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 2)
    train_count, valid_count = partition(
        source, tmp_path / "train.jsonl", tmp_path / "valid.jsonl", 0.05, seed=42
...
def test_partition_ratio_near_one_keeps_train_nonempty(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 5)
    train_count, valid_count = partition(
...
def test_partition_rejects_malformed_line(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 3)
...
def test_partition_leaves_no_tmp_files(tmp_path: Path) -> None:
    source = tmp_path / "source.jsonl"
    jsonl(source, 40)
    partition(source, tmp_path / "train.jsonl", tmp_path / "valid.jsonl", 0.1, 42)
